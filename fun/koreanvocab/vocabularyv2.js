...
// Function to filter words by selected topics
function filterWordsByTopic() {
    const selectedTopics = Array.from(document.querySelectorAll('#topic-selection input:checked'))
                                .map(input => input.value);

    // Filter words based on selected topics
    const filteredWords = words.filter(word => selectedTopics.includes(word.Topic));

    // Shuffle and display filtered words
    if (filteredWords.length > 0) {
        words = filteredWords;
        shuffleWords();
        displayWord();
    } else {
        wordDisplay.textContent = "No words available for the selected topics.";
    }
}

// Event listener for topic selection change
document.querySelectorAll('#topic-selection input').forEach(input => {
    input.addEventListener('change', filterWordsByTopic);
});

// Run the filterWordsByTopic function when the page loads
